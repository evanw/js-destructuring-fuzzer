...
async function main() {
  let es = require('esbuild')
  let esbuild = async (x) => (await es.transform(x, { target: 'es6', loader: 'ts' })).js.trim()

  let ts = require('typescript')
  let typescript = x => ts.transpileModule(x, { compilerOptions: { target: 'es6' } }).outputText.trim()

  let bbl = require('@babel/core')
  let plugins = [['@babel/plugin-transform-typescript', { allowNamespaces: true }], '@babel/plugin-proposal-object-rest-spread']
  let babel = async (x) => (await bbl.transformAsync(x, { plugins })).code.trim()
